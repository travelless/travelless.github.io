...
async function getPosts(pageSize) {
    let paths = await globby(['docs/mdBase/**.md'])
...
`.trim()
            const file = paths + `/docs/blogs/page_${i}.md`
            await fs.writeFile(file, page)
        }
    }
    // rename page_1 to index for homepage
    await fs.move(paths + '/docs/blogs/page_1.md', paths + '/docs/blogs/index.md', { overwrite: true })
}
